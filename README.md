...
— which webOS itself never had, so it is carried across HP's IPC by
`components/input-compat` without changing it — apps opening as cards, db8 with
its schemas loaded, HP's
