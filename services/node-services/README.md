...
addon, carried by `adapters/node-v8-shim`. This component is what replaces
...
```
services/node-services/test/run.sh            # type check + every test
services/node-services/test/run.sh test/luna.fake.test.ts
WEBOS_TEST_LOGS=build/node-services-logs services/node-services/test/run.sh
```
