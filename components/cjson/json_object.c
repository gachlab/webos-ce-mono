...
int json_object_is_type(const struct json_object *this, enum json_type type)
{
  return (this->o_type == type);
}

enum json_type json_object_get_type(const struct json_object *this)
{
  return this->o_type;
}
...
bool json_object_object_get_ex(struct json_object* this, const char *key, struct json_object **value)
{
  JASSERT(this);
  if (!this) return false;
  switch(this->o_type) {
  case json_type_object:
    return lh_table_lookup_ex(this->o.c_object, (void*)key, (void**)value);
  default:
    MC_ERROR("Cannot retrieve object %s from non object %p.  Type is %d\n", key, this, this->o_type);
    JASSERT(FALSE);
    if (value != NULL) {
      *value = NULL;
    }
...
bool json_object_get_boolean(struct json_object *this)
{
  if(!this) return FALSE;
  switch(this->o_type) {
