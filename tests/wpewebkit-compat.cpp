// QtWebKit-shaped API on WPE — WebAppMgr paint/load/input/eval + JS bridge (#81).
...
<html><head><script>
  ...
  document.addEventListener("click", function (e) {
    window.clickedAt = e.clientX + "," + e.clientY;
  });
</script></head>
<body style="margin:0; background:#ff00c8">
  <input id="field" type="email" x-palm-input-type="email"
         style="position:absolute; left:10px; top:10px; width:100px; height:20px">
  <div x-palm-popup-content
       style="position:absolute; left:20px; top:60px; width:50px; height:40px"></div>
</body></html>
)HTML");
    }
    ...
    QObject::connect(page.mainFrame(), &QWebFrame::javaScriptWindowObjectCleared, [&]() {
        ...
        page.mainFrame()->evaluateJavaScript(
            "function greetFromHelper() { return Native.greet('webOS'); }");
    });
    int repaints = 0;
    ...
    failures += report("hitTestContent", hit.isContentEditable()
                       ...
                       QString("editable %1, x-palm-input-type %2")
                           .arg(hit.isContentEditable())
                           .arg(hit.element().attribute("x-palm-input-type")));
    ...
    failures += report("findFirstElement", !popup.isNull() && popup.geometry() == QRect(20, 60, 50, 40),
                       QString("geometry %1,%2 %3x%4")
                           .arg(popup.geometry().x())
                           .arg(popup.geometry().y())
                           .arg(popup.geometry().width())
                           .arg(popup.geometry().height()));
